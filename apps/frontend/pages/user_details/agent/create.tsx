...
const AgentCreatePage = () => {
  ...
  const [superAgents, setSuperAgents] = useState<any[]>([]);
  ...
  useEffect(() => {
    // Fetch all super agents for parent selection
  ...
  const handleParentContinue = () => {
    if (!selectedSuperAgent) {
      alert('Please select a Super Agent');
      return;
    }
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Select Super Agent</h5>
                <button type="button" className="close" onClick={() => router.push('/user_details/agent')}>&times;</button>
              </div>
              <div className="modal-body">
                <select className="form-control" value={selectedSuperAgent} onChange={e => setSelectedSuperAgent(e.target.value)}>
              ...
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={() => router.push('/user_details/agent')}>Close</button>
      ...
      {!showParentDialog && (
        <section className="content">
          <form onSubmit={handleSubmit}>
            ...
            <div className="row mx-0 "><div className="col-12 subowner-cancel-create mb-4 px-0"><button type="button" className="btn btn-secondary" onClick={() => router.push('/user_details/agent')}>Cancel</button><button type="submit" className="btn btn-success float-right" disabled={loading}>{loading ? 'Creating...' : 'Create New Agent'}</button></div></div>
