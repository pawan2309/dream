...
const SubAgentCreatePage = () => {
  ...
  useEffect(() => {
    // Fetch all admins for parent selection
    ...
    try {
      const res = await fetch('/api/users', {
        ...
        body: JSON.stringify({
          ...form,
          role: 'SUB',
          parentId: selectedAdmin,
          creditLimit: Number(form.balance),
        })
      });
      const data = await res.json();
      if (data.success) {
        setSuccess('Sub Agent created successfully! Redirecting...');
        setTimeout(() => router.push('/user_details/sub'), 1500);
      } else {
        setError(data.message || 'Failed to create Sub Agent');
      }
    } catch (err) {
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Select Admin</h5>
                <button type="button" className="close" onClick={() => router.push('/user_details/sub')}>&times;</button>
              </div>
              <div className="modal-body">
                <select className="form-control" value={selectedAdmin} onChange={e => setSelectedAdmin(e.target.value)}>
                  <option value="">Select Admin</option>
                  {admins.map((admin: any) => (
                    <option key={admin.id} value={admin.id}>{admin.code} ({admin.name})</option>
                  ))}
                </select>
              </div>
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={() => router.push('/user_details/sub')}>Close</button>
        ...
        <section className="content-header pb-2">
          <div className="container-fluid">
            <div className="row mb-2">
              <div className="col-sm-6"><h1 className="text-capitalize">Sub Agent</h1></div>
              <div className="col-sm-6">
                <ol className="breadcrumb float-sm-right">
                  <li className="breadcrumb-item"><a href="/">Home</a></li>
                  <li className="breadcrumb-item active text-capitalize">New Sub Agent</li>
                </ol>
              </div>
            </div>
          </div>
      ...
      {!showParentDialog && (
        <section className="content">
          <form onSubmit={handleSubmit}>
            ...
            <div className="row mx-0 "><div className="col-12 subowner-cancel-create mb-4 px-0"><button type="button" className="btn btn-secondary" onClick={() => router.push('/user_details/sub')}>Cancel</button><button type="submit" className="btn btn-success float-right" disabled={loading}>{loading ? 'Creating...' : 'Create New Sub Agent'}</button></div></div>
...
export default SubAgentCreatePage; 