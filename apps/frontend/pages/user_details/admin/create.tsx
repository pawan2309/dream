...
const AdminCreatePage = () => {
  ...
  const [selectedSuperAdmin, setSelectedSuperAdmin] = useState<string>('');
  ...
  useEffect(() => {
    // Fetch all super admins for parent selection
  ...
  const handleParentContinue = () => {
    if (!selectedSuperAdmin) {
      alert('Please select a Superadmin');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setTimeout(() => router.push('/user_details/admin'), 1500);
      } else {
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Select Superadmin</h5>
                <button type="button" className="close" onClick={() => router.push('/user_details/admin')}>&times;</button>
              </div>
              <div className="modal-body">
                <select className="form-control" value={selectedSuperAdmin} onChange={e => setSelectedSuperAdmin(e.target.value)}>
              ...
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={() => router.push('/user_details/admin')}>Close</button>
        ...
        <section className="content-header pb-2">
          <div className="container-fluid">
            <div className="row mb-2">
              <div className="col-sm-6"><h1 className="text-capitalize">Admin</h1></div>
              <div className="col-sm-6">
                <ol className="breadcrumb float-sm-right">
                  <li className="breadcrumb-item"><a href="/">Home</a></li>
                  <li className="breadcrumb-item active text-capitalize">New Admin</li>
                </ol>
              </div>
            </div>
          </div>
      ...
      {!showParentDialog && (
        <section className="content">
          <form onSubmit={handleSubmit}>
            ...
            <div className="row mx-0 "><div className="col-12 subowner-cancel-create mb-4 px-0"><button type="button" className="btn btn-secondary" onClick={() => router.push('/user_details/admin')}>Cancel</button><button type="submit" className="btn btn-success float-right" disabled={loading}>{loading ? 'Creating...' : 'Create New Admin'}</button></div></div>
