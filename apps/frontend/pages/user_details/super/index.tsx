...
export default function SuperPage() {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedUsers(paginatedSupers.map(user => user.id));
    } else {
      setSelectedUsers([]);
    }
  };
  ...
  const filteredSupers = supers.filter(user => 
    user.code?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    user.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    user.contactno?.includes(searchTerm)
  );

  const totalPages = Math.ceil(filteredSupers.length / entriesPerPage);
  const startIndex = (currentPage - 1) * entriesPerPage;
  const endIndex = startIndex + entriesPerPage;
  const paginatedSupers = filteredSupers.slice(startIndex, endIndex);
  ...
  const handleDropdownAction = async (action: string, user: any, e?: React.MouseEvent) => {
    ...
    switch (action) {
      case 'edit':
        window.location.href = `/user_details/super/${user.id}/edit`;
        break;
      case 'active':
      ...
      case 'inactive':
        ...
        break;
              case 'limitUpdate':
          // Navigate to super limit update page
      ...
      case 'sendDevice':
        ...
        break;
      case 'emergency':
        alert(`Super emergency action for: ${user.name}`);
        break;
      case 'statement':
        // Navigate to statement page
        window.location.href = `/user_details/super/statement?userId=${user.id}`;
        break;
      case 'changePassword':
                ...
                <form action="#" method="post" id="demoForm">
                  <div className="card-header">
                    <div className="form-group">
                      <Link href="/user_details/super/create" className="btn btn-primary mr-2">
                        New <i className="fa fa-plus-circle"></i>
                      </Link>
                      <Link href="/user_details/super/limit" className="btn btn-info mr-2">
                    ...
                    {!loading && (
                      <>
                        ...
                        <table className="table table-bordered table-striped">
                            ...
                            <tr>
                              <th>
                                <div style={{ textAlign: 'center' }}>
                                  <input 
                                    type="checkbox" 
                                    checked={selectedUsers.length === paginatedSupers.length && paginatedSupers.length > 0}
                          ...
                          <tbody>
                            {paginatedSupers.length === 0 && (
                              <tr><td colSpan={11} style={{ textAlign: 'center' }}>No super agent users found.</td></tr>
                            )}
                            {paginatedSupers.map((user, idx) => (
                              <tr 
                                key={user.id}
                        ...
                        <div className="row mt-3">
                          <div className="col-sm-6">
                            <p>
                              Showing {startIndex + 1} to {Math.min(endIndex, filteredSupers.length)} of {filteredSupers.length} entries
                            </p>
                          </div>
                          <div className="col-sm-6 text-right">
                            <nav>
