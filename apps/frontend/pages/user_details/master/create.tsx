...
const MasterCreatePage = () => {
  ...
  useEffect(() => {
    // Fetch all sub agents for parent selection
    ...
    try {
      const res = await fetch('/api/users', {
        ...
        body: JSON.stringify({
          ...form,
          role: 'MASTER',
          parentId: selectedSub,
          creditLimit: Number(form.balance),
        })
      });
      const data = await res.json();
      if (data.success) {
        setSuccess('Master created successfully! Redirecting...');
        setTimeout(() => router.push('/user_details/master'), 1500);
      } else {
        setError(data.message || 'Failed to create Master');
      }
    } catch (err) {
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Select Sub Agent</h5>
                <button type="button" className="close" onClick={() => router.push('/user_details/master')}>&times;</button>
              </div>
              ...
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={() => router.push('/user_details/master')}>Close</button>
        ...
        <section className="content-header pb-2">
          <div className="container-fluid">
            <div className="row mb-2">
              <div className="col-sm-6"><h1 className="text-capitalize">Master</h1></div>
              <div className="col-sm-6">
                <ol className="breadcrumb float-sm-right">
                  <li className="breadcrumb-item"><a href="/">Home</a></li>
                  <li className="breadcrumb-item active text-capitalize">New Master</li>
                </ol>
              </div>
            </div>
          </div>
      ...
      {!showParentDialog && (
        <section className="content">
          <form onSubmit={handleSubmit}>
            ...
            <div className="row mx-0 "><div className="col-12 subowner-cancel-create mb-4 px-0"><button type="button" className="btn btn-secondary" onClick={() => router.push('/user_details/master')}>Cancel</button><button type="submit" className="btn btn-success float-right" disabled={loading}>{loading ? 'Creating...' : 'Create New Master'}</button></div></div>
