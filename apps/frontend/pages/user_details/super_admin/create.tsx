    ...
    try {
      const res = await fetch('/api/users', {
        ...
        body: JSON.stringify({
          ...
          role: 'SUPER_ADMIN',
          parentId: undefined // Let backend resolve parent
        })
      });
      const data = await res.json();
      if (data.success) {
        setSuccess('Super Admin created successfully! Redirecting...');
        setTimeout(() => router.push('/user_details/super_admin'), 1500);
      } else {
    ...
    <Layout>
      <section className="content-header pb-2">
        <div className="container-fluid">
          <div className="row mb-2">
            <div className="col-sm-6"><h1 className="text-capitalize">Super Admin</h1></div>
            <div className="col-sm-6">
              <ol className="breadcrumb float-sm-right">
                <li className="breadcrumb-item"><a href="/">Home</a></li>
                <li className="breadcrumb-item active text-capitalize">New Super Admin</li>
              </ol>
            </div>
          </div>
        </div>
      ...
      <section className="content">
        <form onSubmit={handleSubmit}>
          ...
          <div className="row mx-0 "><div className="col-12 subowner-cancel-create mb-4 px-0"><button type="button" className="btn btn-secondary" onClick={() => router.push('/user_details/super_admin')}>Cancel</button><button type="submit" className="btn btn-success float-right" disabled={loading}>{loading ? 'Creating...' : 'Create New Super Admin'}</button></div></div>
