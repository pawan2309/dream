...
export default function AllSuperLedgerPage() {
  ...
  const calculateLenaDenaSummary = () => {
    ...
    filteredSupers.forEach(superAgent => {
      const totalCredit = superAgent.ledger.reduce((sum, entry) => sum + (entry.credit || 0), 0);
      const totalDebit = superAgent.ledger.reduce((sum, entry) => sum + (entry.debit || 0), 0);
      ...
      const sortedLedger = [...superAgent.ledger].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
      const initialCreditEntry = sortedLedger.find(entry => entry.credit > 0);
      const openingBalance = initialCreditEntry ? initialCreditEntry.credit : superAgent.creditLimit;
      ...
      if (netAmount > 0) {
        // This super agent owes us money (Lena He - to receive)
        lenaHe.push(summary);
      } else if (netAmount < 0) {
        // We owe this super agent money (Dena He - to pay)
        denaHe.push(summary);
      }
    });
