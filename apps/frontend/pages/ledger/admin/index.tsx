...
interface AdminWithLedger extends Admin {
  ledger: LedgerEntry[];
}
...
export default function AllAdminLedgerPage() {
  const [siteName, setSiteName] = useState('');
  const [brandName, setBrandName] = useState('');
  const [admins, setAdmins] = useState<AdminWithLedger[]>([]);
  ...
  // Fetch admins and their ledger
  useEffect(() => {
    const fetchAdminsAndLedger = async () => {
      setLoading(true);
      setError('');
      try {
        // Get all admins
        ...
        const adminsWithLedger: AdminWithLedger[] = [];
        
        // Get ledger for each admin
        for (const admin of adminsData.users) {
          try {
            const ledgerRes = await fetch(`/api/users/${admin.id}/ledger`);
            const ledgerData = await ledgerRes.json();
            
            adminsWithLedger.push({
              ...admin,
              ledger: ledgerData.success ? ledgerData.ledger : []
            });
          } catch (err) {
            console.error(`Failed to fetch ledger for admin ${admin.id}:`, err);
            adminsWithLedger.push({
              ...admin,
              ledger: []
            });
  ...
  // Calculate Lena He (payments to receive) and Dena He (payments to pay) for all admins
  const calculateLenaDenaSummary = () => {
    ...
    filteredAdmins.forEach(admin => {
      ...
      const profitLossLedger = admin.ledger.filter(entry => {
      ...
      const summary: LenaDenaSummary = {
        name: `${admin.code} ${admin.name}`,
        contact: admin.contactno || 'N/A',
  ...
  return (
    <Layout>
      <Head>
        <title>All Admin Ledger - {brandName}</title>
        ...
        <div className="container-fluid">
          <div className="row mb-2">
            <div className="col-sm-6">
              <h1>All Admin Ledger</h1>
            </div>
            <div className="col-sm-6">
              <ol className="breadcrumb float-sm-right">
                <li className="breadcrumb-item"><a href="/">Home</a></li>
                <li className="breadcrumb-item active">All Admin Ledger</li>
              </ol>
            </div>
          </div>
            ...
            <div className="col-12">
              <div className="card">
                <div className="card-header">
                  <h3 className="card-title">Admin Ledger Summary</h3>
