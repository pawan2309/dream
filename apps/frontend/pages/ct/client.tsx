...
export default function CashClientPage() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      const response = await fetch(`/api/users/${form.client}/ledger`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/users/${form.client}/manual-ledger`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Refresh clients and transactions to get updated data
        fetchClients();
        fetchTransactions();
      } else {
  ...
  const selectedClientData = clients.find(client => client.id === form.client);
                ...
                <form id="myForm" onSubmit={handleSubmit}>
                  <div className="card-header">
                    ...
                    <div className="form-group col-md-4">
                      <label htmlFor="client" className="text-capitalize">Client</label>
                      <select 
